...
function initscript(context, next) {
  if (context.options.init) {
    var initpath = path.resolve(process.cwd(), context.options.init);
    context.emit('info','init-script-start',initpath);
    fs.exists(initpath, function(exists) {
      if (exists) {
        var proc = child.spawn('node', [initpath], {
        ...
        proc.on('close', function(code) {
          if (code > 0) {
            next(Error('Initialization script failed: ' + code));
            return;
          }
          context.emit('info','init-script-end',initpath);
          next(null, context);
        });
      } else {
        next(Error('Initialization script does not exist!'));
      }
    });
  } else {
    next(null, context);
...
function teardownscript(context, next) {
  if (context.options.teardown) {
    var teardownpath = path.resolve(process.cwd(), context.options.teardown);
    context.emit('info','teardown-script-start',teardownpath);
    fs.exists(teardownpath, function(exists) {
      if (exists) {
        var proc = child.spawn('node', [teardownpath], {
          cwd: path.join(context.path, context.module)
        });
        proc.on('error', function(err) {
          next(err);
        ...
        proc.on('close', function(code) {
          if (code > 0) {
            next(Error('Teardown script failed: ' + code));
            return;
          }
          context.emit('info','teardown-script-end',teardownpath);
          next(null, context);
        });
      } else {
        next(Error('Teardown script does not exist!'));
      }
    });
  } else {
    next(null, context);
...
function npmtest(context, next) {
  if (context.options.script) {
    var scriptpath = path.resolve(process.cwd(), context.options.script);
    context.emit('info','script-start',scriptpath);
    fs.exists(scriptpath, function(exists) {
      if (exists) {
        var proc = child.spawn('node', [scriptpath], {
          cwd: path.join(context.path, context.module),
          stdio: [0,1,2]
        });
        proc.on('error', function(err) {
        ...
        proc.on('close', function(code) {
          ...
          context.emit('info','script-end',scriptpath);
          next(null, context);
        });
      } else {
        next(Error('Test script does not exist!'));
      }
    });
  } else {
...
Tester.prototype.run = function() {
  ...
  async.waterfall([
    ...
    initscript,
    npmsetup,
    npmtest,        // todo, do you even npm test?
    teardownscript
  ], function(err) {
