...
function resolve(context, next) {
  if (context.options.lookup) {
    ...
    context.emit('data','verbose','lookup',detail.name);
    var meta = context.meta;
    if (meta) {
      var rep = lookup[detail.name];
      if (rep) {
        if (rep.replace) {
          context.emit('data','verbose','lookup-found',detail.name);
          var url = makeUrl(
            getRepo(rep.repo, meta),
            detail.rawSpec,
            meta['dist-tags'],
            rep.prefix);
          context.emit('data','verbose','lookup-replace',url);
          context.module.raw = url;
        }
        if (rep.script) {
          context.emit('data','verbose','lookup-script',rep.script);
          context.options.script = rep.script;
        }
      } else {
        context.emit('data','verbose','lookup-notfound',detail.name);
      }
    }
  }
  next(null,context);
